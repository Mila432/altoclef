...
public class CollectCoarseDirtTask extends ResourceTask {
    ...
    protected Task onResourceTick(AltoClef mod) {
        ...
        int dirtCount = mod.getItemStorage().getItemCount(Items.DIRT);
        int gravelCount = mod.getItemStorage().getItemCount(Items.GRAVEL);
        boolean hasEnoughMaterials = (dirtCount >= c) && (gravelCount >= c);
        
        // Critical decision branch: mine vs craft based on resource availability and proximity
        if (!(mod.getItemStorage().getItemCount(Items.DIRT) >= c &&
            ...
            Vec3d playerPos =
                ...
                //#endif
                ;
            boolean withinRange = closest.get().isWithinDistance(playerPos, CLOSE_ENOUGH_COARSE_DIRT);
            if (withinRange) {
